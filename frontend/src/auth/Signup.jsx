...
const Signup = () => {
    ...
    let loginFormHandler = (e) => {
        ...
        let result = userSignupSchema.safeParse(loginFields)
        ...
        <div className="flex items-center justify-center min-h-screen">
            <form onSubmit={loginFormHandler} className="md:P-8 w-full max-w-md rounded-lg md:border border-gray-200 p-3">
                ...
                <div className="relative my-3">
                    <Input value={loginFields.fullname} onChange={setLoginhandler} name='fullname' placeholder="Enter your fullname" type="text" className='pl-8 focus-visible:ring-transparent' />
                    <User className="text-gray-400 absolute inset-y-2 ml-2" />
                    {
                        errors && <span className="text-red-500 text-md">{errors?.fullname}</span>
                    }
                </div>
                ...
                {/* contect */}
                <div className="relative my-3">
                    <Input value={loginFields.contact} onChange={setLoginhandler} name='contact' placeholder="Enter your contact" type="text" className='pl-8 focus-visible:ring-transparent' />
                    <Phone className="text-gray-400 absolute inset-y-2 ml-2" />
                    {
                        errors && <span className="text-red-500 text-md">{errors?.contact}</span>
                    }
                </div>
                ...
                <div className="my-2 text-center">
                    Already have an account? please <Link to={"/login"} className="text-blue-500 cursor-pointer ">Login</Link>
                </div>
            </form>
        </div>
    )
